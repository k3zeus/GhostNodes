...
installer=$(mktemp /tmp/netbird-install.XXXXXX)
trap 'rm -f "$installer"' EXIT
curl -fsSL https://pkgs.netbird.io/install.sh -o "$installer"
bash "$installer"
systemctl enable --now netbird
netbird version
echo 'NetBird instalado. Vincule o peer com: netbird up --setup-key <chave>.'
