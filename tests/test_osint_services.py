...
class OsintContract(unittest.TestCase):
    ...
    def test_swissknife_is_preserved_and_inactive(self):
        ...
        audit_all = (swiss / 'audit-all.sh').read_text()
        self.assertNotIn('services/01-', audit_all)
        self.assertIn('network_discovery.sh', audit_all)
        helper = swiss / 'network_discovery.sh'
        self.assertTrue(helper.is_file())
        helper_text = helper.read_text(encoding='utf-8')
        self.assertNotIn('eth0', helper_text)
        self.assertIn('interface_is_real_up', helper_text)
        self.assertIn('is_halfin_ap', helper_text)
        self.assertIn('select_lan_interface', helper_text)
        self.assertIn('select_scan_range', helper_text)
        self.assertIn('HALFIN_NM_CONF_DIR', helper_text)
        self.assertNotIn(b'\r\n', helper.read_bytes())
        self.assertEqual(subprocess.run([BASH, '-n', str(helper)]).returncode, 0)
        installer = (swiss / 'install.sh').read_text(encoding='utf-8')
        self.assertNotIn('/etc/NetworkManager', installer)
        self.assertNotIn('nmcli ', installer)
        self.assertNotIn('apt -y upgrade', installer)
        self.assertNotIn('/etc/', installer)
    ...
    def test_swissknife_isolated_harness_contract(self):
        ...
        self.assertIn('interface=testlan', text)
        self.assertIn('snapshot after', text)
        self.assertIn('snapshot(){ local n=', text)
        self.assertIn('HALFIN_NM_CONF_DIR=', text)
