...
assert_ok "satoshi uses dedicated systemd service" grep -q "satoshi-bitcoind.service" "$SATOSHI_INSTALL"
assert_ok "satoshi accepts exported version" grep -q "SATOSHI_VERSION" "$SATOSHI_INSTALL"
assert_ok "satoshi accepts exported prune size" grep -q "SATOSHI_PRUNE_GB" "$SATOSHI_INSTALL"
assert_ok "satoshi converts prune GB to MB" grep -q "SATOSHI_PRUNE_GB \\* 1024" "$SATOSHI_INSTALL"

printf "\n${BOLD}  Section: nodenation Satoshi Contract${RESET}\n"
NODENATION="${PROJECT_ROOT}/nodenation"
assert_ok "nodenation keeps direct curl bootstrap spool" grep -q 'exec bash "$_SELF" "$@" < /dev/tty' "$NODENATION"
...
printf "\n${BOLD}  Section: ghostnode/Web Satoshi Contract${RESET}\n"
GHOSTNODE="${PROJECT_ROOT}/ghostnode"
WEB_BITCOIN="${PROJECT_ROOT}/web/backend/routers/bitcoin.py"
assert_ok "ghostnode uses Satoshi service" grep -q "satoshi-bitcoind.service" "$GHOSTNODE"
assert_ok "ghostnode uses bitcoin user datadir" grep -q 'BITCOIN_DIR="/home/${BITCOIN_USER}/.bitcoin"' "$GHOSTNODE"
assert_ok "web API reports Satoshi service" grep -q "satoshi-bitcoind.service" "$WEB_BITCOIN"
assert_ok "web API reads bitcoin-rpc.env" grep -q "bitcoin-rpc.env" "$WEB_BITCOIN"
